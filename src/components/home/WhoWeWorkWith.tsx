...
function IndustryIcon({ type }: { type: (typeof INDUSTRIES)[number]["icon"] }) {
  const cls = "h-8 w-8 stroke-white";
  switch (type) {
    case "building":
      return (
    ...
    <section className="relative overflow-hidden pb-24 pt-32 lg:pb-28 lg:pt-40">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="flex items-center justify-center gap-4">
          <span className="h-px w-16 bg-accent sm:w-24" aria-hidden />
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-5">
            {INDUSTRIES.slice(0, 5).map((item) => (
              <div key={item.label} className="flex flex-col items-center text-center">
                ...
                <p className="mt-3 text-xs font-medium leading-snug text-white/90 sm:text-sm">
                  {item.label}
                </p>
              </div>
            ))}
