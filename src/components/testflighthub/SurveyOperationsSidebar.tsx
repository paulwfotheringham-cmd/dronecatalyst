...
export default function SurveyOperationsSidebar({
  ...
  const inAppNavigation = pathname === "/testflighthub" && onViewChange != null;
    ...
    >
      ...
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-2 py-3 lg:px-3 lg:py-4">
        {surveyNavItems.map((item) => {
          ...
          const active = isSurveyNavItemActive(pathname, item, activeView ?? null);
          const className = cn(
          ...
          return (
            <Link
              key={item.label}
              href={getSurveyNavHref(item.view, item.href)}
              aria-current={active ? "page" : undefined}
              onClick={onClose}
              className={className}
            >
